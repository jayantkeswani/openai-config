...
#   search_result = vector_store.similarity_search_with_score(prompts[field], k=5, max_length=50)
#   print(search_result)
...
for field in prompts:
  ...
  try:
    ...
    retriever = vector_store.as_retriever(search_kwargs={"k": 2, "max_length": 100}, search_type='similarity')
    chain = RetrievalQA.from_chain_type(llm=ChatOpenAI(temperature=0, model=gpt_model),
                                      chain_type="stuff",
                                      retriever=retriever,
                                      return_source_documents=True,
  ...
  for error in error_list:
     if error in result['result']:
      ...
      try:
        ...
        print("length exceeded! Retrying with smaller length")
        retriever = vector_store.as_retriever(search_kwargs={"k": 2, "max_length": 100}, search_type='mmr')
        chain = RetrievalQA.from_chain_type(llm=ChatOpenAI(temperature=0, model=gpt_model),
                                          chain_type="stuff",
                                          retriever=retriever,
                                          return_source_documents=True,
